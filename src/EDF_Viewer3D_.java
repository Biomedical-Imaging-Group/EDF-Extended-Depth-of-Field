...
public class EDF_Viewer3D_ extends JDialog implements ActionListener {
	...
	private JComboBox<String> choiceMap;
	private JComboBox<String> choiceTexture;
	 ...
	 */
	private void addComponent(JPanel pn, int row, final int col, int width, final int height, int space,
			JComponent comp) {
	...
	public synchronized void actionPerformed(ActionEvent e) {
		...
		if (source == bnClose) {
			...
			if (isMapSelected && isTextureSelected) this.bnHeightMap.setEnabled(true);
			else
	...
	private Vector setTextureList(JComboBox choice) {
		...
		for (int i = 0; i < wList.length; i++) {
			...
			if (imp != null) if (imp.getStackSize() == 1) if ((imp.getType() == ImagePlus.COLOR_RGB)
					|| (imp.getType() == ImagePlus.GRAY8) || (imp.getType() == ImagePlus.GRAY16)
					|| (imp.getType() == ImagePlus.GRAY32) || (imp.getType() == ImagePlus.COLOR_256)) {
	...
	private Vector setMapList(JComboBox choice) {
		...
		for (int i = 0; i < wList.length; i++) {
			ImagePlus imp = WindowManager.getImage(wList[i]);
			if (imp != null) if (imp.getStackSize() == 1) if ((imp.getType() == ImagePlus.GRAY32)
					|| (imp.getType() == ImagePlus.GRAY8) || (imp.getType() == ImagePlus.GRAY16)) {
						list.addElement(imp.getTitle());
						index.addElement(new Integer(wList[i]));
					}
